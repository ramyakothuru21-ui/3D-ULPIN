    ...
    <header className="h-16 bg-white border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between gap-4 z-30 sticky top-0 shadow-sm">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <button
          ...
          className="px-3 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl text-xs font-bold shadow-sm flex items-center gap-1.5 transition active:scale-95"
          title="Start 3-Minute SIH Demo Tour"
        >
          <Play className="w-3.5 h-3.5 fill-current" />
          <span className="hidden md:inline">3-Min Demo Tour</span>
        </button>
        ...
        {/* Cadastre Demo Overlay Toggle */}
        <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
          <label className="flex items-center gap-1.5 cursor-pointer text-xs select-none" title="Toggle Simulated AP Meebhoomi Citizen Registry for Hackathon Presentation">
            <input
              type="checkbox"
            ...
            <span className={`text-[11px] font-semibold hidden xl:inline ${enableDemoOverlay ? 'text-brand-700' : 'text-slate-400'}`}>
              Demo Cadastre {enableDemoOverlay && <span className="text-[9px] px-1 py-0.2 bg-amber-100 text-amber-800 font-mono rounded">DEMO</span>}
            </span>
          </label>
        </div>
      </div>
